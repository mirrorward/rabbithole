...
- [~] Account admin transactions (348–355); access-mask projection (big-endian bit order, tested); kick/ban (110/111) — codec landed: `rabbithole-legacy-hotline` gains transactions 350–355 (NewUser/DeleteUser/GetUser/SetUser/UserAccess/UserBroadcast) with wire-value asserts, `access::AccessMask` (8-byte wire form, big-endian bit order `0x80>>(n%8)`, strict decode, 38-variant `Privilege` enum with grant/revoke/has), `UserFlags` (verified vs Mobius/HL1.9: away=0/admin=1/refuse-PM=2/refuse-chat=3 — the folk "bit 0 admin" ordering is off by one), pure `obfuscate`/`deobfuscate` helpers; 42 tests. Wiring into the server's admin handlers + kick/ban flows is the burrow slice
...
- [~] Cross-server file search: signed catalogs, pull fan-out, blake3 dedupe; swarm federated sources live — `rabbithole-federation`: `catalog` (ed25519-signed `SignedCatalog` over CATALOG_CONTEXT‖postcard, `catalog_id = blake3(canonical)`, incremental `generation`+`prev_id` with `supersedes()` staleness, total `from_bytes`), `search` (case-insensitive AND terms, size-range/mime-prefix filters, pagination, provenance-tagged results), `dedupe` (`dedupe_by_hash` collapsing identical blake3 across servers into one match with all `ServerRef` sources), `fanout` (`plan_fetch` FreshestFirst/StableByKey ordering + `max_sources`, `plan_fetch_batch` round-robin). 54 tests. Serving live catalogs over the S2S transport is the server-wiring slice
...
- [x] Feed → board ingestion layer (pure): `mapping` (`to_post_drafts(&Feed, &BoardMapping) -> Vec<PostDraft>` — subject/body/author fallback chains, canonical source link, reuses the blake3 dedup id), `seen` (`partition_fresh` + `SeenSet`, order-preserving, intra-batch repeat handling), `poll` (clockless conditional-GET state machine: `on_response(status, etag, last_modified, ttl, now)` covering 304/2xx/error with capped exponential backoff, TTL/`sy:updatePeriod` as minimum interval, saturating arithmetic). The actual HTTP fetcher + board signing/wiring in burrow is the server slice.
