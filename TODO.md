...
- [~] Listener counts in presence; now-playing surfaced (presence line, TUI status, telnet, web) — server side landed: `PresenceRegistry` gains `RadioStatus` + `set/clear_radio_now_playing`, publishing `ServerEvent::RadioNowPlaying` on the bus. **TUI landed**: `apps/tui/src/radio.rs` — status-bar `♪` segment + Ctrl-N station panel (live-DJ-wins reducer, render-to-lines, truncation; 9 tests), fed today by a documented `[radio]` ServerNotice bridge format with the RADIO-proto decode as the seam. Remaining: server-side notice emission / RADIO proto push projection, telnet + web surfacing
