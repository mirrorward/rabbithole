...
fn main() {
    let out = std::env::args().nth(1).unwrap_or_else(|| "icons.html".into());
    ...
    for path in ["/lobby", "/boards", "/dms", "/directory", "/files", "/radio", "/art", "/admin"] {
        h.push_str(&format!(
    ...
    for (svg, name) in [
        ...
        h.push_str(&format!("<div style='text-align:center'>{svg}<br>{name}</div>"));
    }
    h.push_str("</div></body>");
