...
async fn ctl_client(config: ServerConfig, cmd: &str, args: &[String]) -> Result<()> {
    ...
    let request = match (cmd, args) {
        ...
        ("persona-set", [screen_name, quote]) => {
            json!({"cmd": "persona-set", "screen_name": screen_name, "quote": quote})
        }
        ...
        ("file-area-create", [slug, title, description]) => {
            json!({"cmd": "file-area-create", "slug": slug, "title": title, "description": description})
        }
