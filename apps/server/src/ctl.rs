...
async fn dispatch(shared: &Arc<Shared>, req: &Value) -> Result<Value, String> {
    ...
    match cmd {
        ...
        // ---- Backups (Wave 13) ------------------------------------------
        "backup" => {
            // With no destination, the burrow's own backup folder
            // (`backup_dir`), the one the console uses.
            let dest = match req.get("dest").and_then(Value::as_str) {
                Some(dest) => std::path::PathBuf::from(dest),
                None => crate::backup::backups_dir(&shared.config.read()),
            };
            let outcome = crate::backup::snapshot(shared, &dest)
                .await
                .map_err(|e| format!("{e:#}"))?;
            audit(
                "backup",
        ...
        "peer-approve" => {
            ...
            let origin = req.get("origin").and_then(Value::as_str).map(str::to_owned);
            let (origin, existed) =
                crate::federation::approve_peer(shared, key, origin).map_err(|e| e.to_string())?;
        ...
        "peer-revoke" => {
            let key_hex = str_arg("key")?;
            let key = crate::federation::hex_key(&key_hex).ok_or("key must be 32-byte hex")?;
            let existed = crate::federation::revoke_peer(shared, key).map_err(|e| e.to_string())?;
