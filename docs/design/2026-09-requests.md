...
| 7 | Server-to-server transfers: pick files or folders to send to another burrow you are on, optional swarm, permissions on both sides, clear errors, pieces from participants without exposing the destination | large, security-sensitive | 0.225.0 (burrows) and 0.226.0 (app): the destination-initiated pull over the federation session, approved peers only, filed under the person, trees recreated. 0.227.0: between burrows that are not peers too, when both operators allow it (the destination dials the source's client port, pinned to its certificate, and proves its key). 0.228.0: the swarm leg, when both operators allow it (pieces from the source's seeders, with a capability naming the destination; the source for the rest). Built. [`server-to-server-transfers.md`](server-to-server-transfers.md) |
...
| 9 | Any participant, a person or a burrow, sends the chunks of a file it has, not only whole files; bogus chunks are never accepted | large | 0.229.0 for people: partial seeds serve the units they hold with their Bao proofs (have-maps, `NOT_HELD`), each chunk verified against the file's blake3 root by server and fetcher, the whole file again at the end. 0.230.0 for burrows: the burrow a file is stored on sends proved ranges under a download ticket (FILE 43/44) and joins a person's swarm download as one source among the peers, filling what they lack unit by unit; between burrows the sending burrow sends proved ranges on the pull link, mixed with its seeders (a send with no seeders stays plain, checked whole). Proofs are made in the background the first time, and askers are told to ask again meanwhile. Every source only hands over Bao streams; the fetch checks each against the root itself |
