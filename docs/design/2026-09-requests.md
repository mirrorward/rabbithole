...
| 9 | Any participant, a person or a burrow, sends the chunks of a file it has, not only whole files; bogus chunks are never accepted | large | 0.229.0 for people: partial seeds serve the units they hold with their Bao proofs (have-maps, `NOT_HELD`), each chunk verified against the file's blake3 root by server and fetcher, the whole file again at the end. Burrows as chunk sources next |
