...
| 9 | RADIO | — | now-playing pushes: `RadioNowPlaying` (1), `RadioOff` (2); the station listing: `RadioStationsRequest` (3) → `RadioStations` (4), which carries where the audio is served (`stream_base` / `port`), each station's cover blob id, and its last ten tracks; the operator's view: `RadioStatusRequest` (5) → `RadioStatus` (6), `CONFIG_ADMIN` only, which adds the silent stations, each one's file area, what it is sending (`audio/mpeg` / `audio/ogg` / `audio/flac`), how big its rotation is, and the last twenty tracks it could not play with the reason for each; listener requests: `RadioRequestsRequest` (7) → `RadioRequests` (8), the queue in play order (whether the station takes requests, whether a DJ has the air, which waiting songs are yours and never whose the others are), `RadioRequest` (9) and `RadioRequestVote` (10) → `RadioRequests` (an account that may talk on `radio`, not a guest; three waiting each, fifty a station; refusals `NotFound`, `AlreadyExists` for what is playing, `TooLarge` for the per-person cap, `Unavailable` for a full queue, `RateLimited` for the posting budget), `RadioOfferRequest` (11) → `RadioOffer` (12), what can be asked for as far as a search narrows it, a hundred at a time with how many more: only what the station can send, not what is playing, nothing a moderator is holding back; `RadioRequestsWatch` (13) → `RadioRequestsWatching` (14) selects at most one station per connection (`None` stops), with personalized ephemeral `RadioRequests` (8) pushes on changes; reconnects watch and fetch again, older peers may return `Unsupported` |
