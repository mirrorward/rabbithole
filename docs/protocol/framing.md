...
| 9 | radio | now-playing pushes: `RadioNowPlaying` (1), `RadioOff` (2); station listing `RadioStationsRequest` (3) → `RadioStations` (4): where the audio is served, covers, recent tracks; the operator's `RadioStatusRequest` (5) → `RadioStatus` (6); listener requests `RadioRequestsRequest` (7) → `RadioRequests` (8), `RadioRequest` (9), `RadioRequestVote` (10), `RadioOfferRequest` (11) → `RadioOffer` (12); optional `RadioRequestsWatch` (13) → `RadioRequestsWatching` (14), one station per connection with personalized ephemeral queue pushes (8). Audio itself is the legacy ICY listener (see [`../legacy-surfaces.md`](../legacy-surfaces.md)) |
