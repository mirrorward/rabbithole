...
| 45/46 | FileByContentRequest → FileByContent | Request/Reply | `root` (a blake3 content id) → `{root, node_id, size}`: a file **here** holding that content which this person may download, so a swarm download running on another burrow can open a ticket here and take chunks from this burrow too (0.232). The checks a download ticket makes, minus the ticket: quarantined content is not admitted to exist, and a node is skipped unless the person has `FILE_LIST` **and** `FILE_DOWNLOAD` on its path and may see into its drop box. One content may be filed in several places; the first they may have answers, and at most 32 are looked at. `NotFound` when nothing here holds it **or** nothing they may have does — deliberately the same answer. Charged to the transfer budget (`RateLimited`), so it is no free oracle. Older burrows answer `Unsupported` |
